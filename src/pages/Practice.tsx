      ...
      <Card className="mt-8 p-6">
        ...
        <label className="block">
          ...
          <select
            ...
            className="mt-2 w-full rounded-xl border border-white/10 bg-space-900 px-4 py-3 text-sm text-ink outline-none focus:border-accent/50"
          >
          ...
          <div className="mt-2 grid grid-cols-4 gap-2">
            {[3, 5, 10, 15].map((n) => (
              <button
                key={n}
                onClick={() => setCount(n)}
                className={cn('rounded-xl border px-3 py-2.5 text-sm font-medium transition-all', count === n ? 'border-accent/50 bg-accent/[0.08] text-ink' : 'border-white/10 bg-white/[0.02] text-muted hover:border-white/25')}
              >
                {n}
              </button>
            ))}
...
function Toggle({ active, onClick, icon, title, desc }: { active: boolean; onClick: () => void; icon: React.ReactNode; title: string; desc: string }) {
  return (
    <button onClick={onClick} className={cn('flex items-center gap-3 rounded-xl border px-4 py-3 text-left transition-all', active ? 'border-accent/40 bg-accent/[0.06]' : 'border-white/10 bg-white/[0.02] hover:border-white/20')}>
      <span className={cn('flex h-9 w-9 items-center justify-center rounded-lg border', active ? 'border-accent/30 text-accent-hi' : 'border-white/10 text-muted')}>{icon}</span>
      <span className="flex-1">
        <span className="block text-sm font-medium text-ink">{title}</span>
        <span className="block text-xs text-muted">{desc}</span>
      </span>
      <span className={cn('relative h-6 w-10 rounded-full transition-colors', active ? 'bg-accent' : 'bg-white/10')}>
